...
Route::post('authapi/edit-user', 'UserController@mobileEditProfile')->middleware('TokenAuth');
Route::group([
	...
	$router->resource('Champions',ChampionController::class)->names('ChampionControl');
  
   
    $router->resource('magazine', MagazineController::class)->names('magazineControl');
    $router->resource('banner-magazine', BannerMagazineController::class)->names('BannermagazineControl');
    $router->resource('magazine-topic', MagazineTopicController::class)->names('magazineTopicControl');
    $router->resource('magazine-topic-category', MagazineTopicCategoryController::class)->names('magazineTopicCategoryControl');
    $router->resource('magazine-topic-category', MagazineTopicCategoryController::class)->names('magazineTopicCategoryControl');
    $router->resource('events', EventsController::class)->names('EventControl');
    $router->resource('videos', VideoController::class)->names('videosControl');
    //Config
...
//Shop sprint 3
    ...
    $router->resource('product_price_list', PriceListController::class)->names('PriceListControl');
    $router->resource('user-address', UserAddressController::class)->names('userAddress');
    //companies
...
//Get info
    $router->group(['prefix' => 'get_info'], function ($router) {
        ...
        $router->get('uofm-details', 'PriceListController@getUnitList')->name('uofm-details');
        $router->get('priceInfo', 'PriceListController@getInfoPrice')->name('getInfoPrice');
