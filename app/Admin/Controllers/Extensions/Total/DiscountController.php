...
class DiscountController extends Controller
{
    ...
    protected function grid()
    {
        ...
        if($this->checkSuperUser()){
              $grid->company()->name(trans('admin.company')); 
            }
    ...
    protected function form()
    {
        ...
        $form->number('reward', trans('language.promotion.value'))->default(0)->rules('numeric|min:0');
        ...
        if($this->checkSuperUser()){
           $compaies = Company::where('active', 1)->pluck("name","id");
          $form->select('company_id', trans('language.company'))->options(($compaies));
        }else{
    ...
    public function checkCoupon($code, $companyId){
