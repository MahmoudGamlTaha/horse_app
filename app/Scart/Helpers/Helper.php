...
class Helper
{
    ...
    public static function getListCart($instance, $company_id, $user_id)
    { 
        $cart                = \Cart::instance($company_id, $user_id, $instance);  
        $arrCart['count']    = $cart->count();
        if($instance != 'fav'){
         $arrCart['subtotal'] = $cart->subtotal();//\Helper::currencyRender($cart->subtotal());
        }
        $arrCart['items']    = [];
        ...
        if ($cart->count()) {
            ...
            foreach ( $content as $key => $item) {  
                ...
                $company = $product->company()->first()??null;
                $arrCart['items'][] = [
                    'id'        => intval($item->id),
                    'qty'       => intval($item->qty),
            ...
            $cart->store(md5($cart->getInstance()), $user_id);
        return $arrCart;
    }
