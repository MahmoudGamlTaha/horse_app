...
class ShopProduct extends Model
{
    ...
    public $lang_api;
    public function setLangApi($lang){
        $this->lang_api = (int)$lang;
    }
    ...
    public function getApiUrl(){
        return \Helper::strToUrl('/productDetail/'.$this->company_id.'/'.$this->id.'');
      }
 ...
 */
    ...
    public function getSearch($keyword, $limit = 12, $sortBy = null, $sortOrder = 'asc', $currentlyLang = null)
    {
      ...
      //  $currentlyLang = app()->getLocale();
        ...
        return $this->where('status', 1)
            ...
            ->leftJoin('companies','companies.id', 'shop_product.company_id')
            ...
            })->select(['sku', 'category_id', 'shop_product.company_id','shop_product.id','image','shop_product.path'])
            ->sort($sortBy, $sortOrder)
            ->paginate($limit);
    }
    public function getProductDetails($product_id,$company_id, $currentlyLang = null)
    {
        
      //  $currentlyLang = app()->getLocale();
        $idLang        = $currentlyLang ?? 1;
       // dd($idLang);
       $old = app()->getLocale();
       if($idLang == 2){
           app()->setLocale("en");
       ...
       app()->setlocale("ar");
        return $this->where('status', 1)
            ->where("shop_product.id", $product_id)
            ->where("shop_product.company_id", $company_id)
            ->with('images')
            ...
            ->leftJoin('shop_product_description', 'shop_product_description.product_id', 'shop_product.id')
            ->where('shop_product_description.lang_id', $idLang)
            ->select(['sku', 'category_id','shop_product.type' ,'shop_product.company_id','shop_product.price','shop_product.stock','shop_product.id','image','shop_product.path', 'shop_product.category_id','shop_product.created_at','shop_product.updated_at'])
            ->get();
    }
