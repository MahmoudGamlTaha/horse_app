...
class CartItem implements Arrayable, Jsonable
{
     ...
     * @param array      $options
     */
    public function __construct($user_id, $id, $company_id, $name, $price, array $options = [])
    {
        if(empty($id)) {
        ...
        if(empty($user_id) || !is_numeric($price)){
        ...
        $this->rowId = $this->generateRowId($user_id." ".$company_id, $id, $options);
    }
    ...
    public static function fromBuyableUser($user_id,$company_id, Buyable $item, array $options = [])
    {
        return new self($user_id,$company_id, $item->getBuyableIdentifier($options), $item->getBuyableDescription($options), $item->getBuyablePrice($options), $options);
    }
    /**
     ...
     * @return \Gloudemans\Shoppingcart\CartItem
     */
    public static function fromAttributesUser($user_id, $id, $company_id, $name, $price, array $options = [])
    {
        return new self($user_id, $id, $company_id, $name, $price, $options);
    }
     ...
     * @return string
     */
    protected function generateRowId($user_id ,$id, array $options)
    {
        ksort($options);

        return md5($id . $user_id ."serToMax" .serialize($options));
    }
